...
- **Järgmise etapi plaan ootab kinnitust (13. sept).** Claude'i projektis `claude/jargmine-etapp-plaan.md`. Viis etappi Silveri nelja soovi peale: (1) ühine klassi identiteet `harjutaja_id_v1` alla ja `core/klass.js`, (2) andmebaasi migratsioon 6 — `sessions.module`, tabel `player_modules`, `competed_today`/`report_session`/`class_board` saavad `p_module` vaikeväärtusega, (3) Kirjutaja võistlus, (4) Kell, (5) Kirjutaja uued teemad üks rühm korraga (pikad häälikud → i ja j → ülejäänud). **Võistlusreegel otsustatud (Silver, 13. sept): Kirjutaja võistlus on 20 sõna, 10 sekundit igaüks** (kuni 3 min; Korrutajal 25 × 6 s). Heli tohib võistluses üks kord korrata, aeg jookseb edasi. Plaan muudab ka varasemat reeglit „uus moodul alles siis, kui eelmist kasutatakse" — Kell tuleb enne Kirjutaja v1.1.
