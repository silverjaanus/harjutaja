...
- **Järgmise etapi plaan ootab kinnitust (13. sept).** Claude'i projektis `claude/jargmine-etapp-plaan.md`. Viis etappi Silveri nelja soovi peale: (1) ühine klassi identiteet `harjutaja_id_v1` alla ja `core/klass.js`, (2) andmebaasi migratsioon 6 — `sessions.module`, tabel `player_modules`, `competed_today`/`report_session`/`class_board` saavad `p_module` vaikeväärtusega, (3) Kirjutaja võistlus, (4) Kell, (5) Kirjutaja uued teemad üks rühm korraga (pikad häälikud → i ja j → ülejäänud). **Lahtine võistlusreegel:** mitu sõna ja mitu sekundit Kirjutaja võistluses (ettepanek 20 × 10 s) ning kas heli tohib võistluses uuesti kuulata (ettepanek jah, üks kord). Plaan muudab ka varasemat reeglit „uus moodul alles siis, kui eelmist kasutatakse" — Kell tuleb enne Kirjutaja v1.1.
